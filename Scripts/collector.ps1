param([switch]$Start, [int]$IntervalSeconds = 30)
...
function Invoke-LogCollection {
    ...
    $query = "*[System[(EventID=4624 or EventID=4625 or EventID=4688 or EventID=4720 or EventID=4672 or EventID=4732 or EventID=4722 or EventID=4723 or EventID=4724 or EventID=4698) and EventRecordID > $lastRecordId]]"
    try {
    ...
    if ($new_logs.Count -gt 0) {
        ...
        Write-Host "[$(Get-Date -Format 'HH:mm:ss')] Collected $($new_logs.Count) log(s)"
    } else {
        Write-Host "[$(Get-Date -Format 'HH:mm:ss')] No new logs"
    }
}
...
if ($Start) {
    ...
    Write-Host "FLARE Collector started. Polling every ${IntervalSeconds}s. Press Ctrl+C to stop."
    while ($true) {
        Invoke-LogCollection
        Start-Sleep -Seconds $IntervalSeconds
    }
} else {
    # Single run mode — for Task Scheduler
    Invoke-LogCollection
}